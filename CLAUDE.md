...
- GitHub: remote `git@github.com-liftwod:projectfitcr/liftwod.git` (deploy key `~/.ssh/id_ed25519_liftwod`, SSH alias in `~/.ssh/config`). **Git author trap**: deployments with author `jkarum@gmail.com` get BLOCKED on the Project Fit team — before wiring GitHub auto-deploy, either add that email to the team or set repo-local `git config user.email` to the new account's email.
